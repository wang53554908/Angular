        ...
        restrict: 'EA',
			template: ''+
			      ...
			      '<div class="col-sm-4 text-left">'+
					'<small class="text-muted inline m-t-sm m-b-sm">共{{conf.totalItems}}条记录&nbsp;&nbsp;<select ng-model="conf.itemsPerPage" ng-options="option for option in conf.perPageOptions " ng-change="changeItemsPerPage()"></select></small>'+
				'</div>'+
			        '<div ng-if="conf.totalItems >0" class="col-sm-8 text-right text-right-xs">  '+              
			          '<ul class="pagination pagination-sm m-t-none m-b-none">'+
			            '<li ng-class="{disabled:conf.currentPage == 1}"><a ng-click="prevPage()"><i class="fa fa-chevron-left"></i></a></li>'+
			            '<li ng-repeat="item in pageList track by $index" ng-class="{active:item == conf.currentPage}"><a ng-click="changeCurrentPage(item)">{{item}}</a></li>'+
			            '<li ng-class="{disabled: conf.currentPage == conf.numberOfPages}"><a ng-click="nextPage()"><i class="fa fa-chevron-right"></i></a></li>'+
			          '</ul>'+
			        '</div>'+
			      '</div>'+
			      '',
